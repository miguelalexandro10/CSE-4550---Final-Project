...
class FastballGimmick:
    def __init__(self, ball):
        self.ball = ball  # Reference to the ball, so we can modify its speed
        self.active = False  # Whether the gimmick is currently active
        self.activation_time = 0  # Time when the gimmick was activated

    def activate(self):
        """Activate the speed tripling gimmick."""
        self.active = True
    ...
    def deactivate(self):
        """Deactivate the gimmick, reverting to the original speed."""
        self.active = False
        self.ball.speed_x /= 3
        self.ball.speed_y /= 3

    def update(self):
        """Update gimmick status based on time."""
        current_time = time.time()
        # Check if 20 seconds have passed and the gimmick is not active
        if not self.active and current_time - self.activation_time >= 20:
            self.activate()
        # Check if the gimmick has been active for 5 seconds
        elif self.active and current_time - self.activation_time >= 5:
            self.deactivate()
...
class Game:
    def __init__(self):
        ...
        self.ball = Ball()
        self.fastball = FastballGimmick(self.ball)  # NEW! Initialize the Gimmick with a reference to the ball
